...
enum TrackingAlgorithmStep {
  IsLTFAlgorithm = 0,
  IsCAAlgorithm,
  NTrackingAlgorithmSteps
};

enum McStatus {
  McMatched = 0,
  NotMcMatched,
  CollisionAmongTheCompatibleIds,
...
std::unordered_map<int, float> recoMultiplicity;
std::unordered_map<int, float> recoNumContrib;
...
struct MftReassociationValidation {
  ...
  struct : ConfigurableGroup {
    ...
    Configurable<bool> fillGeneralHistograms{"fillGeneralHistograms", true, "Fill general reassociation efficiencyhistograms"};
    Configurable<bool> fillSpecificHistograms{"fillSpecificHistograms", true, "Fill specific histograms (DCA, DeltaPos)"};
    Configurable<bool> fillParametersHistograms{"fillParametersHistograms", true, "Fill parameters histograms"};
  ...
  struct : ConfigurableGroup {
    ...
    ConfigurableAxis axisDcaX{"axisDcaX", {500, -0.1, 2.1}, "DCAx binning (cm)"};
    ConfigurableAxis axisDcaY{"axisDcaY", {500, -0.1, 2.1}, "DCAy binning (cm)"};
    ConfigurableAxis axisDcaZ{"axisDcaZ", {500, -1., 1.}, "DCAz binning (cm)"};
    ...
    ConfigurableAxis axisPosX{"axisPosX", {500, -1, 1}, "vertex axis for histograms"};
    ConfigurableAxis axisPosZ{"axisPosZ", {500, -30, 30}, "vertex axis for histograms"};
  } configAxis;
  ...
  void init(InitContext&)
  {
    ...
    if (doprocessMcReassociated2d) {
      ...
      if (configTask.fillParametersHistograms) {
  ...
  void processCreateLookupTable(FilteredCollisionsWSelMultMcLabels const& collisions, soa::Join<aod::McCollisions, aod::McCollsExtra> const& mcCollisions)
  {
    ...
    recoVtxY.clear();
    recoVtxZ.clear();
    recoMultiplicity.clear();
    recoNumContrib.clear();
    ...
    recoMultiplicity.reserve(collisions.size());
    recoNumContrib.reserve(collisions.size());
    ...
    for (auto const& col : collisions) {
      ...
      recoMultiplicity.emplace(col.globalIndex(), getMultiplicityEstimator(col));
      recoNumContrib.emplace(col.globalIndex(), col.numContrib());
                               ...
                               aod::BCsWithTimestamps const& /*bcs*/)
  {
    ...
    for (auto const& reassociated2dMftTrack : reassociated2dMftTracks) {
      ...
      if (templatedTrack.has_mcParticle()) {
        ...
        auto iteratorBestCollMult = recoMultiplicity.find(bestRecoColl);
        auto iteratorBestCollNumContrib = recoNumContrib.find(bestRecoColl);
        ...
        if (iteratorBestCollMult == recoMultiplicity.end()) {
          continue;
        }
        if (iteratorBestCollNumContrib == recoNumContrib.end()) {
          continue;
        }
        ...
        const int bestCollMult = iteratorBestCollMult->second;
        const int bestCollNumContrib = iteratorBestCollNumContrib->second;
